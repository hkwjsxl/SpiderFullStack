...
//             return null;
//         var bcV0x = {};
//         for (var a = Hz4D.split("."), l = a.length, i = a[0] == "window" ? 1 : 0; i < l; bcV0x = bcV0x[a[i]] = bcV0x[a[i]] || {},
//             i++)
//             ;
//         return bcV0x
//     }
...
var JW5b = function (i2x, v3x) {
    try {
        ...
        return {}.toString.call(i2x).toLowerCase() == "[object " + v3x + "]"
    } catch (e) {
        return !1
    }
};
...
;
bg3x = function (k2x, cG3x, O3x) {
    if (!k2x || !k2x.length || !gV5a(cG3x))
        return this;
    if (!!k2x.forEach) {
        k2x.forEach(cG3x, O3x);
        return this
    }
    for (var i = 0, l = k2x.length; i < l; i++)
        cG3x.call(O3x, k2x[i], i, k2x);
    return this
}
;
var buV2x = function (chL7E) {
    var m2x = [];
    bg3x(chL7E, function (chE7x) {
        m2x.push(emj[chE7x])
    });
    return m2x.join("")
};
...
var bKB4F = d(JSON.stringify(i2x), buV2x(["流泪", "强"]), buV2x(md), buV2x(["爱心", "女孩", "惊恐", "大笑"]));

console.log(JSON.stringify(i2x))
console.log(buV2x(["流泪", "强"]))
console.log(buV2x(md))
console.log(buV2x(["爱心", "女孩", "惊恐", "大笑"]))
console.log(bKB4F)
